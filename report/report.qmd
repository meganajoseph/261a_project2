...
susp <- ggplot(data=cleaned_schools, aes(x=suspensions_instances, y=grad_rate_midpt)) +
  geom_point() +
...
#| warning: false
#| message: false
#| fig-cap: "Plot (a) shows a scatter plot showing the relationship between the proportion of students enrolled in high level courses including the IB program (purple), AP program (pink), and gifted and talented programs (blue) (x-axis) and graduation rate (y-axis). Plot (b) shows a scatter plot of the graduation rate over the number of students taking at least one AP exam. Plot (c) shows a scatter plot of the graduation rate over the number of students taking either the SAT or ACT."
...
ggplot(cleaned_schools, aes(grad_rate_midpt)) +
  geom_histogram() +
...
# base model
base <- lm(grad_rate_midpt ~ enrollment + title_i_eligible + prop_IB + prop_ap + prop_gat + students_AP_exam_oneormore + students_SAT_ACT + suspensions_instances + teachers_fte_crdc + teachers_first_year_fte + instructional_aides_fte + counselors_fte + salaries_teachers, data=train)

# squre root transformation
sqrt_tranfs <- lm(grad_rate_midpt ~ sqrt(enrollment) + title_i_eligible + sqrt(prop_IB) + sqrt(prop_ap) + sqrt(prop_gat) + sqrt(students_AP_exam_oneormore) + sqrt(students_SAT_ACT) + sqrt(suspensions_instances) + sqrt(teachers_fte_crdc) + sqrt(teachers_first_year_fte) + sqrt(instructional_aides_fte) + sqrt(counselors_fte) + sqrt(salaries_teachers), data=train)

# variable selection using lasso
library(glmnet)
X <- train |> dplyr::select(!grad_rate_midpt) |> as.matrix()
Y <- train$grad_rate_midpt
lasso_res <- cv.glmnet(X, Y, alpha=1)
best_lamda <- lasso_res$lambda.min
best_model <- glmnet(X, Y, alpha=1, lambda=best_lamda)
```
...
The last model created is one using LASSO regression to perform variable selection. LASSO is a shrinkage or regularization regression method that has the ability to "shrink" estimated coefficients to zero, carrying out variable selection. Alongside this, it reduces the variance of parameter estimates. LASSO aims to minimize the equation: $$\sum_{i=1}^n (\boldsymbol{Y}_i - \boldsymbol{X}_i^T\boldsymbol{\beta})^2 + \lambda \sum_{j=1}^{p-1}|\beta_j|$$ where $\lambda \geq 0$ is a tuning parameter that controls the level of regularization, n represents the number of observations, and p is the number of predictors. Essentially, we are minimizing the error sum of squares (SSE) with a penalty term added to it.

I use the `lm()` function from the R programming language [@R] to create the base model and the transformed model. I use the `cv.glmnet()` function from the `glmnet` package to use LASSO regression to select the best model. 
...
Since the goal of this analysis is prediction, the most important assumption that should be met is linearity. After the transformations, there appears to be a linear relationship between the predictors and graduation rate.
...
2.  still confused about interaction, esp how to check, fit a model and see
3.  is there a reason to choose regularization vs stepwise selection for variable selection
4.  how to add interactions in lasso
5.  do i nee to explain what each beta values means for each model
6.  do i need to explain lasso more
7.  is assumption section ok
